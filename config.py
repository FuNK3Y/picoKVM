...
class Config:
    ...
    def save():
        attributes_to_save = {k: v for k, v in Config.__dict__.items(
        ) if not callable(v) and not k.startswith("__") and k != "devices"}
        devices = {"devices": [{"type": type(
            device).__name__, "data": device.to_dict()} for device in Config.devices]}
    ...
    def load():
        ...
        setattr(Config, "devices", [globals()[device["type"]].from_dict(
            device["data"]) for device in content["devices"]])
